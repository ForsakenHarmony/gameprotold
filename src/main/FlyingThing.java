...
//		System.out.println(x + " " + y);
	...
	public void move(){
		v = (xa / Math.cos(Math.toRadians(vdeg)) + ya / Math.sin(Math.toRadians(vdeg)))/2;
		if (degv != 0){
			if (a==0){
				deg = deg + (degv*7);
			}
...
//		System.out.println("fy " + fy);
	...
	public void paint(Graphics2D g){
		g.setTransform(AffineTransform.getRotateInstance(Math.toRadians(deg), fx + Length / 2, fy + Width / 2));
	...
	public Shape getBounds(){
		...
		AffineTransform at = AffineTransform.getRotateInstance(Math.toRadians(deg), fx + Length / 2, fy + Width / 2);
